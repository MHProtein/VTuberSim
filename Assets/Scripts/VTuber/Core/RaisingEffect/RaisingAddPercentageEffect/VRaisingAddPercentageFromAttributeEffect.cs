...
namespace VTuber.Core.RaisingEffect
{
    public class VRaisingAddPercentageFromAttributeEffect : VRaisingEffect, IAttributeEffect
    {
        ...
        public VRaisingAddPercentageFromAttributeEffect(VRaisingAddPercentageFromAttributeEffectConfiguration configuration, string parameter, string upgradedParameter) : base(configuration)
        {
        ...
        public override void ApplyEffect(VCharacter character, Dictionary<string, object> messagedict)
        {
            ...
            if(character.AttributeManager.TryGetAttribute(attributeNameToBeAdded, out var attribute))
            {
                if (attributeNameToBeAdded.Contains("Ability"))
                {
                    ...
                    {
                        abilityAttribute.AddAbility(VMathUtils.FloatToInt(_percentage.Value * attributeToAdd.Value), false);
                    }
                }             else
                {
