...
namespace VTuber.BattleSystem.Core.UI.VCharacterSelection
{
    public class VCharacterSelectionMenu : VUIBehaviour
    {
        ...
        [FormerlySerializedAs("characterDetailsUI")] [SerializeField]
        private VCharacterDetailsUI details;
        ...
        public void Initialize(List<VCharacterConfiguration> characters, Action<VCharacterConfiguration> confirmAction,
            Action returnAction)
        {
            firstTimeShow = true;
        ...
        public void Next()
        {
            ...
            Tween.LocalPosition(details.transform, detailsHiddenPosition.localPosition, .5f).OnComplete(
                () => { details.SetDetails(characters[_index]); }).Chain(Tween
                .LocalPosition(details.transform, detailsPosition.localPosition, 0.5f).OnComplete(
                    () => { confirmButton.interactable = true; }));
        }
        ...
        public void Prev()
        {
            ...
            Tween.LocalPosition(details.transform, detailsHiddenPosition.localPosition, .75f).OnComplete(
                () => { details.SetDetails(characters[_index]); }).Chain(Tween
                .LocalPosition(details.transform, detailsPosition.localPosition, 0.75f).OnComplete(
                    () => { confirmButton.interactable = true; }));
        }
    }
}