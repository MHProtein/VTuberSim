...
namespace VTuber.BattleSystem.Core.UI.VAccountSelection
{
    public class VAccountSelectionCharacterUI : VUIBehaviour
    {
        ...
        private void UpdateUI()
        {
            ...
            singingAbilityGainEfficiencyText.text = "+" + (_character.AttributeManager.Attributes["CASingingAbilityGainEfficiency"].Value - 100) + "%";
            gamingAbilityGainEfficiencyText.text = "+" + (_character.AttributeManager.Attributes["CAGamingAbilityGainEfficiency"].Value - 100) + "%";
            chattingAbilityGainEfficiencyText.text = "+" + (_character.AttributeManager.Attributes["CAChattingAbilityGainEfficiency"].Value - 100) + "%";
        }
    }
}